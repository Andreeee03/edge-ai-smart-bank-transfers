...
OUT_DIR = Path("evaluation/q5_64_server_validation")
DETAILS_PATH = OUT_DIR / "q5_64_500_records.jsonl"
SUMMARY_PATH = OUT_DIR / "q5_64_500_summary.json"
...
# ============================================================
# DIRECT COMPARISON: SERVER 40 vs SERVER 64
# ============================================================

Q5_40_DETAILS_PATH = Path(
    "evaluation/q5_40_server_validation/"
    "q5_40_500_records.jsonl"
)

if Q5_40_DETAILS_PATH.exists():

    records_40 = {
        r["id_example"]: r
        for r in load_jsonl(Q5_40_DETAILS_PATH)
    }
    ...
    assert len(records_40) == 500
    assert len(records_64) == 500
    assert set(records_40) == set(records_64)

    raw_exact_ids = []
    raw_different_ids = []

    alternatives_exact_ids = []
    alternatives_different_ids = []

    token_count_equal_ids = []
    token_count_different_ids = []

    for example_id in records_64:

        r40 = records_40[example_id]
        r64 = records_64[example_id]

        # Exact generated text
        if r40.get("raw_output") == r64.get("raw_output"):
            raw_exact_ids.append(example_id)
        else:
            raw_different_ids.append(example_id)

        # Compare parsed alternatives after normalization
        a40 = [
            normalize_text(x)
            for x in r40.get("predicted_alternatives", [])
        ]

        a64 = [
            normalize_text(x)
            for x in r64.get("predicted_alternatives", [])
        ]

        if a40 == a64:
            alternatives_exact_ids.append(example_id)
        else:
            alternatives_different_ids.append(example_id)

        # Number of generated tokens
        if (
            r40.get("tokens_predicted")
            == r64.get("tokens_predicted")
        ):
            token_count_equal_ids.append(example_id)
        else:
            token_count_different_ids.append(example_id)

    comparison = {
        "records": 500,

        "raw_output_exact_match_count":
            len(raw_exact_ids),

        "raw_output_exact_match_rate":
            len(raw_exact_ids) / 500,

        "raw_output_different_count":
            len(raw_different_ids),

        "raw_output_different_ids":
            raw_different_ids,

        "parsed_alternatives_exact_match_count":
            len(alternatives_exact_ids),

        "parsed_alternatives_exact_match_rate":
            len(alternatives_exact_ids) / 500,

        "parsed_alternatives_different_count":
            len(alternatives_different_ids),

        "parsed_alternatives_different_ids":
            alternatives_different_ids,

        "tokens_predicted_equal_count":
            len(token_count_equal_ids),

        "tokens_predicted_different_count":
            len(token_count_different_ids),

        "tokens_predicted_different_ids":
            token_count_different_ids,
    }

    # Aggiorna anche il summary già creato
    final_summary = json.loads(
        SUMMARY_PATH.read_text(encoding="utf-8")
    )

    final_summary["comparison_q5_server_40_vs_64"] = comparison
    ...
    print("\n" + "=" * 80, flush=True)
    print("Q5 SERVER: 40 vs 64 TOKEN COMPARISON", flush=True)
    print("=" * 80, flush=True)
    ...
    print(
        "\nWARNING: impossibile trovare il precedente "
        "test Q5 server a 40 token.",
        flush=True
    )
